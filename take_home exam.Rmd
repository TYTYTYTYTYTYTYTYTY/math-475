...
numerator <- function(theta,q, data){return(exp(-length(data)*D_n(theta,0.995,data))*dgamma(theta,shape = p.shape,scale = p.scale))}
...
plot(seq(-20,150,by = 0.01),temp_result,pch='.')


#plot(apply(data_frame(c(1:10)),1, function(x){posterior_exam(x,0.995,X,numerator)}))


#posterior_exam(150,0.995,X,numerator)

#numerator(1000,0.995,X)

temp_result[which.max(temp_result)]
...
Fst_dri_posterior(51,0.995,X)
Fst_dri_posterior(52,0.995,X)
...
# tried several learning rate and find 1/5 is okay to use 
graden_descend(40, gradent_neg_posterior, 10^(-4), 10^8, 1/5, 0.995, X)
