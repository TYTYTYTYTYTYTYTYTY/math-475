...
result_tablea <- rbind(lapply(result_poola,function(x){x$'R_xy'}),lapply(result_poola,function(x){x$solution})) %>% 
  t()%>% 
  data.frame() 
...
result_tableb <- rbind(lapply(result_poolb,function(x){x$'R_xy'}),lapply(result_poolb,function(x){x$solution})) %>% 
  t()%>% 
  data.frame() 
...
```{r}
c <- optim(sample(c(0,1),15,replace=T), 
      function(x){rxy(x,A)}, 
...
result_tablec <- rbind(lapply(result_poolc,function(x){x$'value'}),lapply(result_poolc,function(x){x$par})) %>% 
  t()%>% 
  data.frame() 
